...
export class AntiDetectionService {
  private static instance: AntiDetectionService | null = null;
  ...
  private async injectStorageEvasions(page: Page): Promise<void> {
    await page.addInitScript(() => {
      ...
      localStorage.setItem = function(key: string, value: string) {
        if (key.includes('selenium') || key.includes('webdriver')) return;
        return originalSetItem.call(this, key, value);
      };

      const originalGetItem = localStorage.getItem;
      localStorage.getItem = function(key: string): string | null {
        if (key.includes('selenium') || key.includes('webdriver')) return null;
